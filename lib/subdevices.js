...
Messenger.prototype.send = function(data, cb){
  if(data.devices == this.gatewayId && data.subdevice && data.message){

    if(instances[data.subdevice] && instances[data.subdevice].onMessage){
      data.message.fromUuid = this.gatewayId;
      instances[data.subdevice].onMessage(data.message, cb);
    }

  }else{
    this.skynet.message(data);
  }
};
...
function updateRecord(body){
  ...
  .then(function(record){
    ...
    return db.update({name: body.name}, record);
